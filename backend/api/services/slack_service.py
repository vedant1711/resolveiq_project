...
def get_slack_thread_transcript(ticket_id: str) -> Optional[str]:
    if not settings.SLACK_BOT_TOKEN or not settings.SLACK_CHANNEL_ID:
        return None

    messages = _fetch_messages(settings.SLACK_CHANNEL_ID)
    seed = None
    ...
    replies = _fetch_thread(settings.SLACK_CHANNEL_ID, thread_ts)
    lines = [f"Channel: #{settings.SLACK_CHANNEL_ID}"]
    for msg in replies:
