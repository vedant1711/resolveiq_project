...
export default function PendingDraftsTable({ drafts, onRefresh }: PendingDraftsTableProps) {
  ...
  return (
    <Card className="border-border bg-card relative overflow-hidden">
      <span aria-hidden className="hidden dark:block absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-brand-blue/25 to-transparent" />
      <CardHeader className="pb-4 border-b border-border mb-4">
        <CardTitle className="text-base font-semibold flex items-center gap-2">
          ...
          Pending KB Drafts
          {drafts.length > 0 && (
            <Badge className="ml-2 bg-yellow-50 text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400 dark:border dark:border-yellow-500/20 font-medium text-xs">
      ...
      <CardContent>
        {drafts.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground text-sm">
            ...
            <p className="font-medium text-xs">No pending drafts. Generate one from a ticket with insufficient KB coverage.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-sm text-brand-cyan">{draft.ticket_id}</span>
                    <Badge variant="secondary" className="font-medium text-xs bg-yellow-50 text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-400 dark:border dark:border-yellow-500/20">
                      {draft.status === "pending_review" ? "Pending Review" : draft.status}
                    </Badge>
                  </div>
                  <p className="text-sm font-semibold text-slate-800 dark:text-foreground/90 truncate">{draft.generated_title}</p>
                ...
                <div className="flex items-center gap-3 ml-4">
                  {draft.confluence_url && (
                    <a href={draft.confluence_url} target="_blank" rel="noopener noreferrer">
                      <Button size="sm" variant="outline" className="text-xs font-medium border-border hover:bg-slate-50 dark:hover:bg-brand-cyan/10">
                        Review
                      </Button>
                    </a>
                  )}
                  <Button
                    size="sm"
                    variant="default"
                    className="text-xs font-medium bg-brand-blue text-white hover:bg-brand-blue-soft"
