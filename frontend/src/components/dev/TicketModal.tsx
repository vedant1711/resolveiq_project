        ...
        <DialogHeader>
          <div className="flex items-center gap-3 mb-1">
            <a
              ...
              className="font-mono text-sm text-brand-cyan hover:underline transition-colors"
            >
              {ticket.ticket_id}
            </a>
            <Badge variant="outline" className="font-mono text-[10px] uppercase tracking-wider border-border text-muted-foreground">
              {ticket.status}
            </Badge>
          </div>
          <DialogTitle className="font-display text-lg font-semibold pr-8">{ticket.summary}</DialogTitle>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-6">
          ...
          {isAnalyzing && (
            <div className="flex items-center gap-3 p-6 rounded-lg bg-surface border border-border">
              ...
              <div>
                <p className="text-sm font-display font-medium">Analyzing knowledge base coverage...</p>
          ...
          {analyzeResult?.status === "sufficient" && (
            <div className="space-y-4 riq-fade-up">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 dark:bg-brand-cyan/5 border border-green-200 dark:border-brand-cyan/20">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-display text-2xl font-bold text-green-700 dark:text-brand-cyan">{analyzeResult.score}/10</span>
                    <Badge className="bg-green-100 dark:bg-brand-cyan/15 text-green-700 dark:text-brand-cyan border border-green-200 dark:border-brand-cyan/25 font-mono text-[10px] uppercase tracking-wider">
                  ...
                  {analyzeResult.matched_articles.map((article) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium group-hover:text-brand-cyan transition-colors">{article.title}</p>
                        <p className="font-mono text-[10px] text-muted-foreground/60 uppercase tracking-wider">{Math.round(article.relevance * 100)}% match</p>
                      </div>
              ...
              <a href={ticket.jira_url} target="_blank" rel="noopener noreferrer">
                <Button variant="outline" className="w-full mt-2 border-border hover:border-brand-cyan/40 hover:text-brand-cyan font-display text-sm">
                  Open in Jira →
                </Button>
              </a>
            </div>
          ...
          {analyzeResult?.status === "no_articles" && (
            <div className="space-y-4 riq-fade-up">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-surface border border-border">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-display text-xl font-semibold text-muted-foreground">No articles found</span>
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">
          ...
          {analyzeResult?.status === "insufficient" && (
            <div className="space-y-4 riq-fade-up">
              <div className="flex items-center gap-3 p-4 rounded-lg riq-critical-pulse border border-red-200 dark:border-red-500/20">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-display text-2xl font-bold text-red-600 dark:text-red-400">{analyzeResult.score}/10</span>
                    <Badge className="bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-500/25 font-mono text-[10px] uppercase tracking-wider">
              ...
              {!generatedDraft && (
                <Button
                  ...
                  size="lg"
                  className="w-full h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-display font-semibold tracking-wide transition-all duration-200 dark:riq-cta-glow dark:font-bold dark:uppercase dark:tracking-[0.15em]"
                >
                  {isGenerating ? (
                    <span className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-xs normal-case tracking-wide">{loadingPhase}</span>
                    </span>
                  ) : (
                    <span className="flex items-center gap-2">
              ...
              {generatedDraft && (
                <div className="riq-fade-up p-5 rounded-lg bg-green-50 dark:bg-brand-cyan/5 border border-green-200 dark:border-brand-cyan/20 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="font-display font-semibold text-green-700 dark:text-brand-cyan">Draft created in Confluence!</p>
                  </div>
                  ...
                  {generatedDraft.confluence_url && (
                    <a href={generatedDraft.confluence_url} target="_blank" rel="noopener noreferrer">
                      <Button
                        variant="outline"
                        className="w-full mt-1 border-green-300 dark:border-brand-cyan/30 text-green-700 dark:text-brand-cyan hover:bg-green-50 dark:hover:bg-brand-cyan/10 font-display text-sm"
                      >
